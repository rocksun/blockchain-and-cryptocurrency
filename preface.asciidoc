...
IBM认为区块链的核心就是公共账本，于是Hyperledger Fabric项目的目标就变成了构建互信的私链。为了实现传统的企业安全需求，Fabric的运行架构变得异常复杂，以至于要另起一个Composer项目对此进行支持。而以太坊这类框架，却似乎有着非常单调的架构，是否并不适合构建企业级的私链？可能看了太多虚无缥缈的东西，起初，我对IBM关于区块链的理解，感到非常认同，但是复杂的架构和相对于以太坊似乎是不是有些自废武功，不过，这一切却使我不得不反思，这些区块链项目是解决的同一个问题吗？

比特币是人类的伟大发明，天然的稀缺性决定了它不会像法币一样不停的膨胀，政府无法像法币一样通过比特币暗自盘剥老百姓，正是抱着这种理想，许多人义无反顾地加入到了币圈。还有许多人，通过铸币的公平看到了更多的可能，所以又有了更多各式各样的区块链应用。我感觉，其中可能是有一个更高层次的问题，这个问题的答案也许不应该叫区块链，也许是对区块链的更深入的定义。为此，有了这本书。
